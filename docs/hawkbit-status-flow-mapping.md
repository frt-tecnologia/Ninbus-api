...
| artifact.info `type` (case-sensitive) | SM type key | DS type key | canonical data member | destination |
|--------------------------------------|-------------|-------------|-----------------------|-------------|
| `firmware-ninbus` | `firmware-ninbus` | `ninbus-firmware-ninbus` | `data/firmware.npm` (signed manifest + image) | STM32 flash via bootloader |
| `firmware-controller` | `firmware-controller` | `ninbus-firmware-controller` | `data/controller.fir` | LightDot via CAN |
| `configuration-nfx` | `configuration-nfx` | `ninbus-configuration-nfx` | `data/config.frz` | LightDot NAND via CAN |
...
- The device does **not** read the DS/SM type from hawkBit. It identifies the
  update path solely from `artifact.info` (`type "<type>"`) inside the `.tar`
  — the **canonical v4 format** (tools/ota_pack.py, repo Ninbus-v4) — and
  branches in its install switch-case. `tar-validator.ts` enforces the exact
  structure server-side; `tar-packager.ts` emits it for controller/NFX.

### Tar packaging (canonical v4 — generated by tools/ota_pack.py, repo Ninbus-v4)

GOLDEN RULE: the device NEVER accepts raw .bin/.hex/.elf — only the signed
canonical TAR below. Anything else is rejected with closed+failure
("REJEITADO: manifesto sem magic NPM" — deployment 657).

```
update-app.tar        ← USTAR, mtime=0, deterministic (NOT .tar.gz)
├── artifact.info          → 'type "firmware-ninbus"'  (first member)
└── data/firmware.npm      → 128 B signed NPM manifest (magic NPM,
                          SHA-256(image‖counter‖size), ECDSA P-256 DER 8–72 B,
                          padding 0xFF) + image ≤ 192 KiB
```

The server NEVER signs: firmware-ninbus releases must arrive as the
ota_pack.py-generated `.tar` (validated by `src/modules/firmware/tar-validator.ts`
and stored VERBATIM); raw .bin uploads are rejected with 400 INVALID_EXTENSION.
firmware-controller also accepts a raw `.fir` (packaged server-side into the
canonical format by `tar-packager.ts`).
