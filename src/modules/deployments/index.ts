			...
			try {
				...
				await logActivity({
					...
					metadata: {
						...
						targetCount: deployment.targetsAssigned ?? 0,
					},
				});
				...
				if (error instanceof HawkbitApiError) {
					if (error.status === 409) {
						...
						return {
							error: 'Conflict',
							message: 'Distribution Set collision in hawkBit — please retry.',
						};
					}
					appLogger.error(
						'[DEPLOYMENTS] hawkBit API error: %s',
					...
					return {
						error: 'Bad Gateway',
						message: `hawkBit returned error (${error.status}): ${detail}`,
					};
				}
				if (
				...
				return {
					...
					message: `Deployment creation failed: ${error?.message ?? 'unknown error'}`,
				};
			}
		},
		{
	...
	.get(
		...
		async ({ params, query, set }) => {
			try {
				...
				appLogger.warn(
					'[DEPLOYMENTS] Failed to list: %s',
					error instanceof Error ? error.message : String(error),
				);
				set.status = 503;
				return {
					error: 'Service Unavailable',
					message: 'Deployment service (hawkBit) is currently unavailable',
				};
			}
		},
		{
			...
			detail: {
				tags: ['Deployments'],
				summary: 'List deployments (Distribution Sets) for this company',
			},
			response: {
	...
	.delete(
		...
		async ({ params, user, set }) => {
			try {
				...
				return {
					error: 'Service Unavailable',
					message: `Failed to delete deployment: ${error?.message ?? 'hawkBit error'}`,
				};
			}
		},
		{
